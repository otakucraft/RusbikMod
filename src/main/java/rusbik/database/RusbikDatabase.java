...
// Toda la gesti贸n de bases de datos.
public class RusbikDatabase {
    ...
    // Se ejecuta al a帽adir al jugador a la whitelist desde el comando !add de discord.
    public static void addPlayerInformation(String name, long discordId) throws SQLException {
        if (c != null){
            ...
            ResultSet rsAddPlayer = psAddPlayer.executeQuery();
            rsAddPlayer.close();
            psAddPlayer.close();
            ...
            ResultSet rsPlayerPos = psPlayerPos.executeQuery();
            rsPlayerPos.close();
            psPlayerPos.close();
            
            //A馻dir el id de discord
            ...
            ResultSet rsDiscId = psDiscId.executeQuery();
            rsDiscId.close();
            psDiscId.close();
    ...
    // Conseguir la posici贸n de la 煤ltima muerte.
    ...
    // Check de si este jugador est谩 registrado en la base de datos con nombre y discord ID.
    ...
    // Banear usuario para que no pueda meter a m谩s gente.
...
//    }
    ...
    // Si tiene permitido eliminar. Para que desde el comando !remove nadie elimine que no sea a s铆 mismo.
    ...
    // Acci贸n al eliminarte de la whitelist.
    ...
    // Check de si el jugador ya ha registrado alg煤n usuario, solo puedes registrar una cuenta.
    ...
    // Actualizar n煤mero cada vez que te conectas, solo se usa para comprobar si es la primera vez que te unes.
    ...
    // Extraer la informaci贸n de un bloque especificado por coordenadas, dimensi贸n, y la p谩gina.
    ...
    // N煤mero de p谩ginas que puede tener de historial el bloque.
    ...
    // Extraer todos los IDs de gente no baneada o a帽adida por excepci贸n.
    ...
    // Elimina registros del logor si se supera un n鷐ero de registros
